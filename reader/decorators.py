    ...
    def decorator(request, *args, **kwargs):
        ...
        try:
            if isinstance(objects, QuerySet):
                ...
                data = simplejson.dumps(objects,encoding='utf-8', ensure_ascii=False, cls=DjangoJSONEncoder)
            if 'callback' in request.REQUEST:
                # a jsonp response!
                data = '%s(%s);' % (request.REQUEST['callback'], data)
                return HttpResponse(data, "application/javascript")
        except:
            data = simplejson.dumps(str(objects), encoding='utf-8',ensure_ascii=False, cls=DjangoJSONEncoder)
